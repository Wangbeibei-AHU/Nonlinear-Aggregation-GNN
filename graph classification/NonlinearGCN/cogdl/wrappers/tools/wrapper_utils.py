...
def split_dataset(ndata, train_ratio, test_ratio, kfold):
    ...
    index = np.arange(ndata)
    random.shuffle(index)
    idx = int(ndata * test_ratio)
    test_index = index[idx*kfold:idx*(kfold+1)]
    val_index = test_index
    train_index = np.concatenate([index[:idx*kfold],index[idx*(kfold+1):]])
