...
from cogdl.layers import GINGATLayer
from cogdl.utils import split_dataset_general
from cogdl.utils import get_activation

class FC(nn.Module):
    def __init__(self, in_feats, out_features, head, bias=True):
        super(FC, self).__init__()
        self.w = nn.Parameter(torch.zeros(size=(head, in_feats, out_features)))
        self.bias = nn.Parameter(torch.FloatTensor(head,1,out_features))
...
class MLP(nn.Module):
    def __init__(
        ...
        self.mlp = nn.ModuleList(
            [FC(shapes[layer], shapes[layer + 1], head, bias=bias) for layer in range(num_layers)]
        )
...
class GINGAT(BaseModel):
    ...
    def __init__(
        ...
        for i in range(num_layers - 1):
            if i == 0:
                mlp = MLP(in_feats, hidden_dim, hidden_dim, num_mlp_layers, norm="batchnorm",head=head[i])
                self.gin_layers.append(GINGATLayer(mlp, eps, train_eps,in_feats,head[i]))
            else:
                mlp = MLP(hidden_dim*head[i-1], hidden_dim, hidden_dim, num_mlp_layers, norm="batchnorm",head=head[i])
                self.gin_layers.append(GINGATLayer(mlp, eps, train_eps,hidden_dim*head[i-1],head[i]))
            self.batch_norm.append(nn.BatchNorm1d(hidden_dim*head[i]))
        ...
        for i in range(self.num_layers):
            if i == 0:
                ...
                self.linear_prediction.append(nn.Linear(hidden_dim*head[i-1], out_feats))
