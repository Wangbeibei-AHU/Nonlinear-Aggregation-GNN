...
|-------------------------------------{'-' * (len(str(args.dataset)) + len(model_name) + len(dw_name) + len(mw_name))}|"""
    ...
    # Go!!!
    result, recode_loss, recode_acc = trainer.run(model_wrapper, dataset_wrapper)
    

    return result, recode_loss, recode_acc
...
def raw_experiment(args):
    ...
    # pdb.set_trace()
    for variant, result in zip(variants, results[0]):
    ...
    return results[0][0]['test_acc'], results[0][1]
